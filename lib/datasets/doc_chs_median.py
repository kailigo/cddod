...
class doc_chs_median(imdb):
    def __init__(self, image_set, devkit_path=None):
        imdb.__init__(self, 'doc_chs_median_' + image_set)
        self._year = '2007'
        self._image_set = image_set
        self._devkit_path = cfg_d.DOC_CHS_MEDIAN #self._get_default_path() if devkit_path is None \
...
#            else devkit_path
    ...
    def ps_image_path_at(self, i):
        """
        Return the absolute path to image i in the image sequence.
        """
        return self.ps_image_path_from_index(self._image_index[i])
    ...
    def ps_image_path_from_index(self, index):

        image_path = os.path.join(self._data_path,'ps_text_raster',
                                  index + self._image_ext)
        assert os.path.exists(image_path), \
            'Path does not exist: {}'.format(image_path)
        return image_path
    ...
    def _load_pascal_annotation(self, index):
        ...
        for ix, obj in enumerate(objs):
            ...
            if cls_text == 'footnote':
                cls_text = 'text'
            cls = self._class_to_ind[cls_text]
